...
import { ReviewModelResponseError } from "../ai/review-model.js";
import type {
  ReviewInput,
...
import type { ReviewFinding, ReviewWorkQueue } from "./review-run.js";

export class ReviewRunProcessor {
  constructor(
  ...
  async process(reviewRunId: string): Promise<void> {
    const run = this.store.claimQueuedReviewRun(reviewRunId);
    if (run === null) {
      return;
    }
    ...
    const reviewReportId = this.store.completeReviewRun(
      reviewRunId,
      this.model.name,
      storedFindings,
      randomUUID(),
  ...
  private fail(reviewRunId: string, errorCode: ReviewRunErrorCode, startedAt: number): void {
    this.store.failReviewRun(reviewRunId, errorCode);
    this.logger.warn(
      {
        durationMs: elapsedMilliseconds(startedAt),
        errorCode,
...
function isValidFinding(finding: ReviewFinding, chunk: ReviewInput["chunks"][number]): boolean {
  return (
    hasValidContent(finding) && (chunk.changedLines.get(finding.path)?.has(finding.line) ?? false)
  );
}
...
function toStoredFindings(findings: readonly ReviewFinding[]): readonly FindingInput[] {
